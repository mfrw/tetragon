...
func initEnv(t *testing.T, cancelWg *sync.WaitGroup) (*Grpc, context.CancelFunc) {
	...
	lEventCache := eventcache.NewWithTimer(lServer, time.Millisecond*50)
	lExecCache := execcache.New(lServer)
...
func TestGrpcExecOutOfOrder(t *testing.T) {
	...
	time.Sleep(200 * time.Millisecond) // wait for cache to do it's work
